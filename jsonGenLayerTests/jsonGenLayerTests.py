...
def RunTest(executable_path, SHADERS_PATH):
    ...
    p = subprocess.run(command_list)
    return p.stdout, p.stderr, p.returncode

def CompareJsonwithGolden(json_gen_path, golden_json_path):
    file_list = sorted(os.listdir(json_gen_path))
    file_list_golden = sorted(os.listdir(golden_json_path))

    if file_list != file_list_golden:
        print("file set generated by test does not match golden file set")
        print("    generated by test: " + str(file_list));
        print("    golden file set:   " + str(file_list_golden));
        return False
...
def main():
    ...
    stdout, stderr, returncode = RunTest(executable_path, SHADERS_PATH)
    if returncode == 0:
        print("JSON_GEN_LAYER_TESTS ==> DONE")
    else:
        print("JSON_GEN_LAYER_TESTS ==> FAILED (exited with return code " + str(returncode) + ")")
        exit(1)
