...
class VkTraceFileOutputGenerator(OutputGenerator):
    ...
    def GenerateTraceVkSource(self):
        ...
        trace_vk_src += '    for (int sIndex = 0; sIndex < 8 && !gMessageStream; sIndex++) {\n'
        trace_vk_src += '        gMessageStream = vktrace_MessageStream_create(FALSE, ipAddr, VKTRACE_BASE_PORT + VKTRACE_TID_VULKAN + sIndex);\n'
        trace_vk_src += '    }\n'
        trace_vk_src += '    if (!gMessageStream) {\n'
        trace_vk_src += '        vktrace_LogError("Cannot connect to trace server\\n");\n'
        trace_vk_src += '        return; }\n'
        trace_vk_src += '#endif\n'
