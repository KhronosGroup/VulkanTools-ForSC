...
class ApiDumpSubcommand(Subcommand):
    def generate_header(self):
        ...
        header_txt.append('')
        header_txt.append('#ifdef __ANDROID__')
        header_txt.append('#include "android/log.h"')
        ...
        header_txt.append('')
        header_txt.append('// Helper class to forward the cout/cerr to logcat derived from:')
        header_txt.append('// http://stackoverflow.com/questions/8870174/is-stdcout-usable-in-android-ndk')
        header_txt.append('// https://github.com/googlesamples/vulkan-basic-samples/blob/master/LunarGSamples/API-Samples/utils/util.cpp')
        header_txt.append('class AndroidBuffer : public std::streambuf {')
        header_txt.append('public:')
        header_txt.append('    AndroidBuffer(android_LogPriority priority) {')
        header_txt.append('        priority_ = priority;')
        header_txt.append('        this->setp(buffer_, buffer_ + kBufferSize - 1);')
        header_txt.append('    }')
        header_txt.append('private:')
        header_txt.append('    static const int32_t kBufferSize = 128;')
        header_txt.append('    int32_t overflow(int32_t c) {')
        header_txt.append('        if (c == traits_type::eof()) {')
        header_txt.append('            *this->pptr() = traits_type::to_char_type(c);')
        header_txt.append('            this->sbumpc();')
        header_txt.append('        }')
        header_txt.append('        return this->sync() ? traits_type::eof() : traits_type::not_eof(c);')
        header_txt.append('    }')
        header_txt.append('')
        header_txt.append('    int32_t sync() {')
        header_txt.append('        int32_t rc = 0;')
        header_txt.append('        if (this->pbase() != this->pptr()) {')
        header_txt.append('            char writebuf[kBufferSize + 1];')
        header_txt.append('            memcpy(writebuf, this->pbase(), this->pptr() - this->pbase());')
        header_txt.append('            writebuf[this->pptr() - this->pbase()] = \'\\0\';')
        header_txt.append('')
        header_txt.append('            rc = __android_log_write(priority_, "std", writebuf) > 0;')
        header_txt.append('            this->setp(buffer_, buffer_ + kBufferSize - 1);')
        header_txt.append('        }')
        header_txt.append('        return rc;')
        header_txt.append('    }')
        header_txt.append('')
        header_txt.append('    android_LogPriority priority_ = ANDROID_LOG_INFO;')
        header_txt.append('    char buffer_[kBufferSize];')
        header_txt.append('};')
        header_txt.append('')
        ...
        header_txt.append('    {')
        header_txt.append('#ifdef __ANDROID__')
        header_txt.append('        std::cout.rdbuf(new AndroidBuffer(ANDROID_LOG_INFO));')
        header_txt.append('#endif')
        header_txt.append('        outputStream = &std::cout;')
        header_txt.append('    }')
        header_txt.append('')
