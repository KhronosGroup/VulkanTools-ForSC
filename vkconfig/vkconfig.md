...
# Vulkan Confiuration Tool (vkconfig)
...
The layer manager is a graphical user interface (GUI), and does not support any functionality through the system console. It may be launched from the console (as `vkconfig`), but no further functionality will be available from the console.
...
Finally, at the bottom of the layer manager, there are four buttons to control the tool: "Save", "Restore", "Clear", and "Exit".
...
**Lenny's Note:** This button's implementation is incomplete so you won't see any effect from this until the next revision
