...
vktrace_trace_packet_header *vkCreateCommandPool(
    bool makeCall, VkDevice device, const VkCommandPoolCreateInfo *pCreateInfo,
    ...
    if (makeCall) {
        result = mdd(device)->devTable.CreateCommandPool(
    ...
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pCreateInfo),
        sizeof(VkCommandPoolCreateInfo), pCreateInfo);
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pAllocator),
                                       sizeof(VkAllocationCallbacks), NULL);
    vktrace_add_buffer_to_trace_packet(pHeader,
                                       (void **)&(pPacket->pCommandPool),
                                       sizeof(VkCommandPool), pCommandPool);
...
vkAllocateCommandBuffers(bool makeCall, VkDevice device,
    ...
    CREATE_TRACE_PACKET(vkAllocateCommandBuffers,
                        get_struct_chain_size((void *)pAllocateInfo) +
    ...
    if (makeCall) {
        ...
        // valid handle.
        result = mdd(device)->devTable.AllocateCommandBuffers(
    ...
    pPacket->device = device;
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pAllocateInfo),
        sizeof(VkCommandBufferAllocateInfo), pAllocateInfo);
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pCommandBuffers),
        sizeof(VkCommandBuffer) * pAllocateInfo->commandBufferCount,
        pCommandBuffers);
    pPacket->result = result;
    vktrace_finalize_buffer_address(pHeader,
                                    (void **)&(pPacket->pAllocateInfo));
    vktrace_finalize_buffer_address(pHeader,
                                    (void **)&(pPacket->pCommandBuffers));
    vktrace_finalize_trace_packet(pHeader);
    return pHeader;
}
...
//=====================================================================
vktrace_trace_packet_header *
vkBeginCommandBuffer(bool makeCall, VkCommandBuffer commandBuffer,
                     const VkCommandBufferBeginInfo *pBeginInfo) {
    VkResult result = VK_SUCCESS;
    vktrace_trace_packet_header *pHeader;
    packet_vkBeginCommandBuffer *pPacket = NULL;
    CREATE_TRACE_PACKET(vkBeginCommandBuffer,
                        get_struct_chain_size((void *)pBeginInfo));

    if (makeCall) {
        mdd(commandBuffer)
            ->devTable.BeginCommandBuffer(commandBuffer, pBeginInfo);
    }

    vktrace_set_packet_entrypoint_end_time(pHeader);
    pPacket = interpret_body_as_vkBeginCommandBuffer(pHeader);
    pPacket->commandBuffer = commandBuffer;
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pBeginInfo),
                                       sizeof(VkCommandBufferBeginInfo),
                                       pBeginInfo);
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pBeginInfo->pInheritanceInfo),
...
vktrace_trace_packet_header *vkCmdPipelineBarrier(
    bool makeCall, VkCommandBuffer commandBuffer,
    ...
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pMemoryBarriers),
        memoryBarrierCount * sizeof(VkMemoryBarrier), pMemoryBarriers);
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pBufferMemoryBarriers),
        bufferMemoryBarrierCount * sizeof(VkBufferMemoryBarrier),
        pBufferMemoryBarriers);
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pImageMemoryBarriers),
        imageMemoryBarrierCount * sizeof(VkImageMemoryBarrier),
        pImageMemoryBarriers);

    vktrace_finalize_buffer_address(pHeader,
                                    (void **)&(pPacket->pMemoryBarriers));
    vktrace_finalize_buffer_address(pHeader,
                                    (void **)&(pPacket->pBufferMemoryBarriers));
    vktrace_finalize_buffer_address(pHeader,
                                    (void **)&(pPacket->pImageMemoryBarriers));
    vktrace_finalize_trace_packet(pHeader);
    return pHeader;
}

//=====================================================================
vktrace_trace_packet_header *vkEndCommandBuffer(bool makeCall,
...
//=====================================================================
vktrace_trace_packet_header *vkQueueSubmit(bool makeCall, VkQueue queue,
    ...
    pPacket->fence = fence;
    pPacket->result = result;
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pSubmits),
                                       submitCount * sizeof(VkSubmitInfo),
                                       pSubmits);
    for (i = 0; i < submitCount; ++i) {
        vktrace_add_buffer_to_trace_packet(
            pHeader, (void **)&(pPacket->pSubmits[i].pCommandBuffers),
            pPacket->pSubmits[i].commandBufferCount * sizeof(VkCommandBuffer),
            pSubmits[i].pCommandBuffers);
        vktrace_finalize_buffer_address(
            pHeader, (void **)&(pPacket->pSubmits[i].pCommandBuffers));
        vktrace_add_buffer_to_trace_packet(
            pHeader, (void **)&(pPacket->pSubmits[i].pWaitSemaphores),
            pPacket->pSubmits[i].waitSemaphoreCount * sizeof(VkSemaphore),
            pSubmits[i].pWaitSemaphores);
        vktrace_finalize_buffer_address(
            pHeader, (void **)&(pPacket->pSubmits[i].pWaitSemaphores));
        vktrace_add_buffer_to_trace_packet(
            pHeader, (void **)&(pPacket->pSubmits[i].pSignalSemaphores),
            pPacket->pSubmits[i].signalSemaphoreCount * sizeof(VkSemaphore),
            pSubmits[i].pSignalSemaphores);
        vktrace_finalize_buffer_address(
            pHeader, (void **)&(pPacket->pSubmits[i].pSignalSemaphores));
        vktrace_add_buffer_to_trace_packet(
            pHeader, (void **)&(pPacket->pSubmits[i].pWaitDstStageMask),
...
//=====================================================================
vktrace_trace_packet_header *vkResetCommandPool(bool makeCall, VkDevice device,
    ...
    if (makeCall) {
        result =
            mdd(device)->devTable.ResetCommandPool(device, commandPool, flags);
    }
...
vkFreeCommandBuffers(bool makeCall, VkDevice device, VkCommandPool commandPool,
    ...
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pCommandBuffers),
        commandBufferCount * sizeof(VkCommandBuffer), pCommandBuffers);
    vktrace_finalize_buffer_address(pHeader,
                                    (void **)&(pPacket->pCommandBuffers));
    vktrace_finalize_trace_packet(pHeader);
    return pHeader;
...
//=====================================================================
vktrace_trace_packet_header *
vkDestroyCommandPool(bool makeCall, VkDevice device, VkCommandPool commandPool,
    ...
    if (makeCall) {
        ...
        mdd(device)->devTable.DestroyCommandPool(device, commandPool,
                                                 pAllocator);
    }
...
//=====================================================================
vktrace_trace_packet_header *vkMapMemory(bool makeCall, VkDevice device,
    ...
    if (makeCall) {
        result = mdd(device)->devTable.MapMemory(device, memory, offset, size,
                                                 flags, &(*ppData));
    }
    ...
    pPacket->size = size;
    pPacket->flags = flags;
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->ppData),
                                       sizeof(void *), *ppData);
...
//=====================================================================
vktrace_trace_packet_header *vkUnmapMemory(bool makeCall, VkDeviceSize size,
                                           void *pData, VkDevice device,
                                           VkDeviceMemory memory) {
    vktrace_trace_packet_header *pHeader;
    ...
    if (size > 0) {
        vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pData),
                                           size, pData);
...
//=====================================================================
vktrace_trace_packet_header *
vkCreateBuffer(bool makeCall, VkDevice device,
    ...
    CREATE_TRACE_PACKET(vkCreateBuffer,
                        get_struct_chain_size((void *)pCreateInfo) +
                            sizeof(VkAllocationCallbacks) + sizeof(VkBuffer));

    if (makeCall) {
        result = mdd(device)->devTable.CreateBuffer(device, pCreateInfo,
                                                    pAllocator, pBuffer);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
    ...
    pPacket->device = device;
    vktrace_add_buffer_to_trace_packet(pHeader,
                                       (void **)&(pPacket->pCreateInfo),
                                       sizeof(VkBufferCreateInfo), pCreateInfo);
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pCreateInfo->pQueueFamilyIndices),
        sizeof(uint32_t) * pCreateInfo->queueFamilyIndexCount,
        pCreateInfo->pQueueFamilyIndices);
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pAllocator),
                                       sizeof(VkAllocationCallbacks), NULL);
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pBuffer),
                                       sizeof(VkBuffer), pBuffer);
    pPacket->result = result;
    vktrace_finalize_buffer_address(
        pHeader, (void **)&(pPacket->pCreateInfo->pQueueFamilyIndices));
    vktrace_finalize_buffer_address(pHeader, (void **)&(pPacket->pCreateInfo));
...
//=====================================================================
vktrace_trace_packet_header *
vkGetBufferMemoryRequirements(bool makeCall, VkDevice device, VkBuffer buffer,
                              VkMemoryRequirements *pMemoryRequirements) {
    vktrace_trace_packet_header *pHeader;
    packet_vkGetBufferMemoryRequirements *pPacket = NULL;
    CREATE_TRACE_PACKET(vkGetBufferMemoryRequirements,
                        sizeof(VkMemoryRequirements));
    if (makeCall) {
        mdd(device)->devTable.GetBufferMemoryRequirements(device, buffer,
                                                          pMemoryRequirements);
    }
    ...
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pMemoryRequirements),
        sizeof(VkMemoryRequirements), pMemoryRequirements);
    vktrace_finalize_buffer_address(pHeader,
                                    (void **)&(pPacket->pMemoryRequirements));
    vktrace_finalize_trace_packet(pHeader);
    return pHeader;
}
...
vktrace_trace_packet_header *vkAllocateMemory(
    bool makeCall, VkDevice device, const VkMemoryAllocateInfo *pAllocateInfo,
    ...
    if (makeCall) {
        result = mdd(device)->devTable.AllocateMemory(device, pAllocateInfo,
                                                      pAllocator, pMemory);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
    ...
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pAllocateInfo),
        sizeof(VkMemoryAllocateInfo), pAllocateInfo);
    add_alloc_memory_to_trace_packet(pHeader,
                                     (void **)&(pPacket->pAllocateInfo->pNext),
                                     pAllocateInfo->pNext);
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pAllocator),
                                       sizeof(VkAllocationCallbacks), NULL);
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pMemory),
                                       sizeof(VkDeviceMemory), pMemory);
    pPacket->result = result;
    vktrace_finalize_buffer_address(pHeader,
                                    (void **)&(pPacket->pAllocateInfo));
    vktrace_finalize_buffer_address(pHeader, (void **)&(pPacket->pAllocator));
...
//=====================================================================
vktrace_trace_packet_header *vkBindBufferMemory(bool makeCall, VkDevice device,
                                                VkBuffer buffer,
                                                VkDeviceMemory memory,
    ...
    if (makeCall) {
        result = mdd(device)->devTable.BindBufferMemory(device, buffer, memory,
                                                        memoryOffset);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
//=====================================================================
vktrace_trace_packet_header *
vkCmdCopyBuffer(bool makeCall, VkCommandBuffer commandBuffer,
    ...
    if (makeCall) {
        mdd(commandBuffer)
            ->devTable.CmdCopyBuffer(commandBuffer, srcBuffer, dstBuffer,
    ...
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pRegions),
                                       regionCount * sizeof(VkBufferCopy),
                                       pRegions);
...
//=====================================================================
vktrace_trace_packet_header *
vkCreateFence(bool makeCall, VkDevice device,
    ...
    CREATE_TRACE_PACKET(vkCreateFence,
                        get_struct_chain_size((void *)pCreateInfo) +
                            sizeof(VkAllocationCallbacks) + sizeof(VkFence));
    if (makeCall) {
        result = mdd(device)->devTable.CreateFence(device, pCreateInfo,
                                                   pAllocator, pFence);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
    ...
    vktrace_add_buffer_to_trace_packet(pHeader,
                                       (void **)&(pPacket->pCreateInfo),
                                       sizeof(VkFenceCreateInfo), pCreateInfo);
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pAllocator),
                                       sizeof(VkAllocationCallbacks), NULL);
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pFence),
                                       sizeof(VkFence), pFence);
    pPacket->result = result;
    vktrace_finalize_buffer_address(pHeader, (void **)&(pPacket->pCreateInfo));
    vktrace_finalize_buffer_address(pHeader, (void **)&(pPacket->pAllocator));
...
//=====================================================================
vktrace_trace_packet_header *
vkWaitForFences(bool makeCall, VkDevice device, uint32_t fenceCount,
    ...
    if (makeCall) {
        result = mdd(device)->devTable.WaitForFences(device, fenceCount,
    ...
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pFences),
                                       fenceCount * sizeof(VkFence), pFences);
    pPacket->result = result;
...
//=====================================================================
vktrace_trace_packet_header *vkResetFences(bool makeCall, VkDevice device,
                                           uint32_t fenceCount,
...
//=====================================================================
vktrace_trace_packet_header *
vkDestroyBuffer(bool makeCall, VkDevice device, VkBuffer buffer,
                const VkAllocationCallbacks *pAllocator) {
    vktrace_trace_packet_header *pHeader;
...
//=====================================================================
vktrace_trace_packet_header *
vkDestroyBufferView(bool makeCall, VkDevice device, VkBufferView bufferView,
                    const VkAllocationCallbacks *pAllocator) {
    vktrace_trace_packet_header *pHeader;
...
//=====================================================================
vktrace_trace_packet_header *
vkFreeMemory(bool makeCall, VkDevice device, VkDeviceMemory memory,
             const VkAllocationCallbacks *pAllocator) {
    vktrace_trace_packet_header *pHeader;
...
//=====================================================================
vktrace_trace_packet_header *
vkAllocateDescriptorSets(bool makeCall, VkDevice device,
    ...
    if (makeCall) {
        result = mdd(device)->devTable.AllocateDescriptorSets(
    ...
    CREATE_TRACE_PACKET(
        ...
        vk_size_vkdescriptorsetallocateinfo(pAllocateInfo) +
    ...
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pAllocateInfo),
        sizeof(VkDescriptorSetAllocateInfo), pAllocateInfo);
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pAllocateInfo->pSetLayouts),
        pPacket->pAllocateInfo->descriptorSetCount *
            sizeof(VkDescriptorSetLayout),
        pAllocateInfo->pSetLayouts);
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pDescriptorSets),
        pPacket->pAllocateInfo->descriptorSetCount * sizeof(VkDescriptorSet),
        pDescriptorSets);
    pPacket->result = result;
    vktrace_finalize_buffer_address(
        pHeader, (void **)&(pPacket->pAllocateInfo->pSetLayouts));
    vktrace_finalize_buffer_address(pHeader,
                                    (void **)&(pPacket->pDescriptorSets));
    vktrace_finalize_buffer_address(pHeader,
                                    (void **)&(pPacket->pAllocateInfo));
    vktrace_finalize_trace_packet(pHeader);
    return pHeader;
...
vktrace_trace_packet_header *vkUpdateDescriptorSets(
    bool makeCall, VkDevice device, uint32_t descriptorWriteCount,
    ...
    if (makeCall) {
        mdd(device)->devTable.UpdateDescriptorSets(
    ...
    // begin custom code
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pDescriptorWrites),
    ...
    for (i = 0; i < descriptorWriteCount; i++) {
        ...
        case VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT: {
            vktrace_add_buffer_to_trace_packet(
                pHeader, (void **)&(pPacket->pDescriptorWrites[i].pImageInfo),
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER: {
            vktrace_add_buffer_to_trace_packet(
                pHeader,
                (void **)&(pPacket->pDescriptorWrites[i].pTexelBufferView),
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC: {
            vktrace_add_buffer_to_trace_packet(
                pHeader, (void **)&(pPacket->pDescriptorWrites[i].pBufferInfo),
                pDescriptorWrites[i].descriptorCount *
...
//=====================================================================
vktrace_trace_packet_header *
vkResetDescriptorPool(bool makeCall, VkDevice device,
    ...
    if (makeCall) {
        result = mdd(device)->devTable.ResetDescriptorPool(
...
//=====================================================================
vktrace_trace_packet_header *vkCmdBeginQuery(bool makeCall,
                                             VkCommandBuffer commandBuffer,
    ...
    if (makeCall) {
        mdd(commandBuffer)
            ->devTable.CmdBeginQuery(commandBuffer, queryPool, query, flags);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
vktrace_trace_packet_header *vkCmdEndQuery(bool makeCall,
    ...
    if (makeCall) {
        mdd(commandBuffer)
            ->devTable.CmdEndQuery(commandBuffer, queryPool, query);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
vkCmdWriteTimestamp(bool makeCall, VkCommandBuffer commandBuffer,
    ...
    if (makeCall) {
        mdd(commandBuffer)
            ->devTable.CmdWriteTimestamp(commandBuffer, pipelineStage,
                                         queryPool, query);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
vktrace_trace_packet_header *vkCmdResetQueryPool(bool makeCall,
    ...
    if (makeCall) {
        mdd(commandBuffer)
            ->devTable.CmdResetQueryPool(commandBuffer, queryPool, firstQuery,
                                         queryCount);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
vkDestroyFramebuffer(bool makeCall, VkDevice device, VkFramebuffer framebuffer,
    ...
    if (makeCall) {
        mdd(device)->devTable.DestroyFramebuffer(device, framebuffer,
                                                 pAllocator);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
vkDestroyDescriptorPool(bool makeCall, VkDevice device,
    ...
    if (makeCall) {
        mdd(device)->devTable.DestroyDescriptorPool(device, descriptorPool,
                                                    pAllocator);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
vktrace_trace_packet_header *vkCreateRenderPass(
    ...
    uint32_t attachmentCount =
        (pCreateInfo != NULL && (pCreateInfo->pAttachments != NULL))
            ? pCreateInfo->attachmentCount
            : 0;
    uint32_t dependencyCount =
        (pCreateInfo != NULL && (pCreateInfo->pDependencies != NULL))
            ? pCreateInfo->dependencyCount
            : 0;
    uint32_t subpassCount =
        (pCreateInfo != NULL && (pCreateInfo->pSubpasses != NULL))
    ...
    // end custom code
    if (makeCall) {
        result = mdd(device)->devTable.CreateRenderPass(
    ...
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pCreateInfo),
        sizeof(VkRenderPassCreateInfo), pCreateInfo);
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pCreateInfo->pAttachments),
        attachmentCount * sizeof(VkAttachmentDescription),
        pCreateInfo->pAttachments);
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pCreateInfo->pDependencies),
        dependencyCount * sizeof(VkSubpassDependency),
        pCreateInfo->pDependencies);
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pCreateInfo->pSubpasses),
        subpassCount * sizeof(VkSubpassDescription), pCreateInfo->pSubpasses);
    uint32_t i;
    for (i = 0; i < pPacket->pCreateInfo->subpassCount; i++) {
        VkSubpassDescription *pSubpass =
            (VkSubpassDescription *)&pPacket->pCreateInfo->pSubpasses[i];
        const VkSubpassDescription *pSp = &pCreateInfo->pSubpasses[i];
        vktrace_add_buffer_to_trace_packet(
            pHeader, (void **)&(pSubpass->pInputAttachments),
            pSubpass->inputAttachmentCount * sizeof(VkAttachmentReference),
            pSp->pInputAttachments);
        vktrace_finalize_buffer_address(
            pHeader, (void **)&(pSubpass->pInputAttachments));
        vktrace_add_buffer_to_trace_packet(
            pHeader, (void **)&(pSubpass->pColorAttachments),
            pSubpass->colorAttachmentCount * sizeof(VkAttachmentReference),
            pSp->pColorAttachments);
        vktrace_finalize_buffer_address(
            pHeader, (void **)&(pSubpass->pColorAttachments));
        vktrace_add_buffer_to_trace_packet(
            pHeader, (void **)&(pSubpass->pResolveAttachments),
            pSubpass->colorAttachmentCount * sizeof(VkAttachmentReference),
            pSp->pResolveAttachments);
        vktrace_finalize_buffer_address(
            pHeader, (void **)&(pSubpass->pResolveAttachments));
        vktrace_add_buffer_to_trace_packet(
            pHeader, (void **)&(pSubpass->pDepthStencilAttachment),
            1 * sizeof(VkAttachmentReference), pSp->pDepthStencilAttachment);
        vktrace_finalize_buffer_address(
            pHeader, (void **)&(pSubpass->pDepthStencilAttachment));
        vktrace_add_buffer_to_trace_packet(
            pHeader, (void **)&(pSubpass->pPreserveAttachments),
    ...
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pAllocator),
                                       sizeof(VkAllocationCallbacks), NULL);
    vktrace_add_buffer_to_trace_packet(pHeader,
                                       (void **)&(pPacket->pRenderPass),
                                       sizeof(VkRenderPass), pRenderPass);
    pPacket->result = result;
    vktrace_finalize_buffer_address(
        pHeader, (void **)&(pPacket->pCreateInfo->pAttachments));
    vktrace_finalize_buffer_address(
...
vktrace_trace_packet_header *vkCreateGraphicsPipelines(
    ...
    CREATE_TRACE_PACKET(vkCreateGraphicsPipelines,
                        total_size + sizeof(VkAllocationCallbacks) +
                            createInfoCount * sizeof(VkPipeline));
    if (makeCall) {
        result = mdd(device)->devTable.CreateGraphicsPipelines(
    ...
    vktrace_add_buffer_to_trace_packet(
        pHeader, (void **)&(pPacket->pCreateInfos),
        createInfoCount * sizeof(VkGraphicsPipelineCreateInfo), pCreateInfos);
    add_VkGraphicsPipelineCreateInfos_to_trace_packet(
        pHeader, (VkGraphicsPipelineCreateInfo *)pPacket->pCreateInfos,
        pCreateInfos, createInfoCount);
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pAllocator),
                                       sizeof(VkAllocationCallbacks), NULL);
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pPipelines),
                                       createInfoCount * sizeof(VkPipeline),
                                       pPipelines);
    pPacket->result = result;
...
vktrace_trace_packet_header *vkCreateComputePipelines(
    ...
    CREATE_TRACE_PACKET(vkCreateComputePipelines,
                        createInfoCount * sizeof(VkComputePipelineCreateInfo) +
                            getVkComputePipelineCreateInfosAdditionalSize(
                                createInfoCount, pCreateInfos) +
                            sizeof(VkAllocationCallbacks) +
                            createInfoCount * sizeof(VkPipeline));

    if (makeCall) {
        result = mdd(device)->devTable.CreateComputePipelines(
            device, pipelineCache, createInfoCount, pCreateInfos, pAllocator,
            pPipelines);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
    ...
    add_VkComputePipelineCreateInfos_to_trace_packet(
        pHeader, (VkComputePipelineCreateInfo *)pPacket->pCreateInfos,
        pCreateInfos, createInfoCount);
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pAllocator),
                                       sizeof(VkAllocationCallbacks), NULL);
    vktrace_add_buffer_to_trace_packet(pHeader, (void **)&(pPacket->pPipelines),
...
vkDestroyPipelineCache(bool makeCall, VkDevice device,
    ...
    if (makeCall) {
        mdd(device)->devTable.DestroyPipelineCache(device, pipelineCache,
                                                   pAllocator);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
vkDestroyPipelineLayout(bool makeCall, VkDevice device,
    ...
    if (makeCall) {
        mdd(device)->devTable.DestroyPipelineLayout(device, pipelineLayout,
                                                    pAllocator);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
vkDestroyShaderModule(bool makeCall, VkDevice device,
    ...
    if (makeCall) {
        mdd(device)->devTable.DestroyShaderModule(device, shaderModule,
                                                  pAllocator);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
vkDestroyDescriptorSetLayout(bool makeCall, VkDevice device,
    ...
    CREATE_TRACE_PACKET(vkDestroyDescriptorSetLayout,
                        sizeof(VkAllocationCallbacks));
    if (makeCall) {
        mdd(device)->devTable.DestroyDescriptorSetLayout(
...
vkDestroySwapchainKHR(bool makeCall, VkDevice device, VkSwapchainKHR swapchain,
    ...
    if (makeCall) {
        mdd(device)->devTable.DestroySwapchainKHR(device, swapchain,
                                                  pAllocator);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
vkDestroySurfaceKHR(bool makeCall, VkInstance instance, VkSurfaceKHR surface,
    ...
    if (makeCall) {
        mid(instance)->instTable.DestroySurfaceKHR(instance, surface,
                                                   pAllocator);
    }
    vktrace_set_packet_entrypoint_end_time(pHeader);
...
vkCmdCopyBufferToImage(bool makeCall, VkCommandBuffer commandBuffer,
    ...
    CREATE_TRACE_PACKET(vkCmdCopyBufferToImage,
                        regionCount * sizeof(VkBufferImageCopy));

    if (makeCall) {
        mdd(commandBuffer)
            ->devTable.CmdCopyBufferToImage(commandBuffer, srcBuffer, dstImage,
