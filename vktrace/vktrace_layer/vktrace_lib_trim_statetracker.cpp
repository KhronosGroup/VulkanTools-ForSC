...
void StateTracker::clear() {
    while (createdInstances.size() != 0) {
        remove_Instance(
            reinterpret_cast<VkInstance>(createdInstances.begin()->first));
    }
    ...
    while (createdDevices.size() != 0) {
        remove_Device(reinterpret_cast<VkDevice>(createdDevices.begin()->first));
    }

    while (createdSurfaceKHRs.size() != 0) {
        remove_SurfaceKHR(
            reinterpret_cast<VkSurfaceKHR>(createdSurfaceKHRs.begin()->first));
    }

    while (createdCommandPools.size() != 0) {
        remove_CommandPool(
            reinterpret_cast<VkCommandPool>(createdCommandPools.begin()->first));
    }

    while (createdCommandBuffers.size() != 0) {
        remove_CommandBuffer(
            reinterpret_cast<VkCommandBuffer>(createdCommandBuffers.begin()->first));
    }

    while (createdDescriptorPools.size() != 0) {
        remove_DescriptorPool(reinterpret_cast<VkDescriptorPool>(
            createdDescriptorPools.begin()->first));
    }

    while (createdRenderPasss.size() != 0) {
        remove_RenderPass(
            reinterpret_cast<VkRenderPass>(createdRenderPasss.begin()->first));
    }

    while (createdPipelineCaches.size() != 0) {
        remove_PipelineCache(
            reinterpret_cast<VkPipelineCache>(createdPipelineCaches.begin()->first));
    }

    while (createdPipelines.size() != 0) {
        remove_Pipeline(
            reinterpret_cast<VkPipeline>(createdPipelines.begin()->first));
    }

    while (createdQueues.size() != 0) {
        remove_Queue(reinterpret_cast<VkQueue>(createdQueues.begin()->first));
    }

    while (createdSemaphores.size() != 0) {
        remove_Semaphore(
            reinterpret_cast<VkSemaphore>(createdSemaphores.begin()->first));
    }

    while (createdDeviceMemorys.size() != 0) {
        remove_DeviceMemory(
            reinterpret_cast<VkDeviceMemory>(createdDeviceMemorys.begin()->first));
    }

    while (createdFences.size() != 0) {
        remove_Fence(reinterpret_cast<VkFence>(createdFences.begin()->first));
    }

    while (createdSwapchainKHRs.size() != 0) {
        remove_SwapchainKHR(
            reinterpret_cast<VkSwapchainKHR>(createdSwapchainKHRs.begin()->first));
    }

    while (createdImages.size() != 0) {
        remove_Image(reinterpret_cast<VkImage>(createdImages.begin()->first));
    }

    while (createdImageViews.size() != 0) {
        remove_ImageView(
            reinterpret_cast<VkImageView>(createdImageViews.begin()->first));
    }

    while (createdBuffers.size() != 0) {
        remove_Buffer(reinterpret_cast<VkBuffer>(createdBuffers.begin()->first));
    }

    while (createdBufferViews.size() != 0) {
        remove_BufferView(
            reinterpret_cast<VkBufferView>(createdBufferViews.begin()->first));
    }

    while (createdFramebuffers.size() != 0) {
        remove_Framebuffer(
            reinterpret_cast<VkFramebuffer>(createdFramebuffers.begin()->first));
    }

    while (createdEvents.size() != 0) {
        remove_Event(reinterpret_cast<VkEvent>(createdEvents.begin()->first));
    }

    while (createdQueryPools.size() != 0) {
        remove_QueryPool(
            reinterpret_cast<VkQueryPool>(createdQueryPools.begin()->first));
    }

    while (createdShaderModules.size() != 0) {
        remove_ShaderModule(
            reinterpret_cast<VkShaderModule>(createdShaderModules.begin()->first));
    }

    while (createdPipelineLayouts.size() != 0) {
        remove_PipelineLayout(reinterpret_cast<VkPipelineLayout>(
            createdPipelineLayouts.begin()->first));
    }

    while (createdSamplers.size() != 0) {
        remove_Sampler(reinterpret_cast<VkSampler>(createdSamplers.begin()->first));
    }

    while (createdDescriptorSetLayouts.size() != 0) {
        remove_DescriptorSetLayout(reinterpret_cast<VkDescriptorSetLayout>(
            createdDescriptorSetLayouts.begin()->first));
    }

    while (createdDescriptorSets.size() != 0) {
        remove_DescriptorSet(
            reinterpret_cast<VkDescriptorSet>(createdDescriptorSets.begin()->first));
    }
...
ObjectInfo *StateTracker::add_Instance(VkInstance var) {
    ObjectInfo &info = createdInstances[reinterpret_cast<uint64_t>(var)];
...
ObjectInfo *StateTracker::add_PhysicalDevice(VkPhysicalDevice var) {
    ObjectInfo &info = createdPhysicalDevices[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_Device(VkDevice var) {
    ObjectInfo &info = createdDevices[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_SurfaceKHR(VkSurfaceKHR var) {
    ObjectInfo &info = createdSurfaceKHRs[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_CommandPool(VkCommandPool var) {
    ObjectInfo &info = createdCommandPools[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_CommandBuffer(VkCommandBuffer var) {
    ObjectInfo &info = createdCommandBuffers[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_DescriptorPool(VkDescriptorPool var) {
    ObjectInfo &info = createdDescriptorPools[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_RenderPass(VkRenderPass var) {
    ObjectInfo &info = createdRenderPasss[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_PipelineCache(VkPipelineCache var) {
    ObjectInfo &info = createdPipelineCaches[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_Pipeline(VkPipeline var) {
    ObjectInfo &info = createdPipelines[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_Queue(VkQueue var) {
    ObjectInfo &info = createdQueues[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_Semaphore(VkSemaphore var) {
    ObjectInfo &info = createdSemaphores[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_DeviceMemory(VkDeviceMemory var) {
    ObjectInfo &info = createdDeviceMemorys[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_Fence(VkFence var) {
    ObjectInfo &info = createdFences[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_SwapchainKHR(VkSwapchainKHR var) {
    ObjectInfo &info = createdSwapchainKHRs[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_Image(VkImage var) {
    ObjectInfo &info = createdImages[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_ImageView(VkImageView var) {
    ObjectInfo &info = createdImageViews[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_Buffer(VkBuffer var) {
    ObjectInfo &info = createdBuffers[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_BufferView(VkBufferView var) {
    ObjectInfo &info = createdBufferViews[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_Framebuffer(VkFramebuffer var) {
    ObjectInfo &info = createdFramebuffers[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_Event(VkEvent var) {
    ObjectInfo &info = createdEvents[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_QueryPool(VkQueryPool var) {
    ObjectInfo &info = createdQueryPools[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_ShaderModule(VkShaderModule var) {
    ObjectInfo &info = createdShaderModules[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_PipelineLayout(VkPipelineLayout var) {
    ObjectInfo &info = createdPipelineLayouts[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_Sampler(VkSampler var) {
    ObjectInfo &info = createdSamplers[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_DescriptorSetLayout(VkDescriptorSetLayout var) {
    ObjectInfo &info = createdDescriptorSetLayouts[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::add_DescriptorSet(VkDescriptorSet var) {
    ObjectInfo &info = createdDescriptorSets[reinterpret_cast<uint64_t>(var)];
    memset(&info, 0, sizeof(ObjectInfo));
    info.vkObject = (uint64_t)var;
    return &info;
}
...
ObjectInfo *StateTracker::get_Instance(VkInstance var) {
    TrimObjectInfoMap::iterator iter = createdInstances.find(reinterpret_cast<uint64_t>(var));
...
ObjectInfo *StateTracker::get_PhysicalDevice(VkPhysicalDevice var) {
    TrimObjectInfoMap::iterator iter = createdPhysicalDevices.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_Device(VkDevice var) {
    TrimObjectInfoMap::iterator iter = createdDevices.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_SurfaceKHR(VkSurfaceKHR var) {
    TrimObjectInfoMap::iterator iter = createdSurfaceKHRs.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_Queue(VkQueue var) {
    TrimObjectInfoMap::iterator iter = createdQueues.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_SwapchainKHR(VkSwapchainKHR var) {
    TrimObjectInfoMap::iterator iter = createdSwapchainKHRs.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_CommandPool(VkCommandPool var) {
    TrimObjectInfoMap::iterator iter = createdCommandPools.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_CommandBuffer(VkCommandBuffer var) {
    TrimObjectInfoMap::iterator iter = createdCommandBuffers.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_DeviceMemory(VkDeviceMemory var) {
    TrimObjectInfoMap::iterator iter = createdDeviceMemorys.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_ImageView(VkImageView var) {
    TrimObjectInfoMap::iterator iter = createdImageViews.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_Image(VkImage var) {
    TrimObjectInfoMap::iterator iter = createdImages.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_BufferView(VkBufferView var) {
    TrimObjectInfoMap::iterator iter = createdBufferViews.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_Buffer(VkBuffer var) {
    TrimObjectInfoMap::iterator iter = createdBuffers.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_Sampler(VkSampler var) {
    TrimObjectInfoMap::iterator iter = createdSamplers.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_DescriptorSetLayout(VkDescriptorSetLayout var) {
    TrimObjectInfoMap::iterator iter = createdDescriptorSetLayouts.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_PipelineLayout(VkPipelineLayout var) {
    TrimObjectInfoMap::iterator iter = createdPipelineLayouts.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_RenderPass(VkRenderPass var) {
    TrimObjectInfoMap::iterator iter = createdRenderPasss.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_ShaderModule(VkShaderModule var) {
    TrimObjectInfoMap::iterator iter = createdShaderModules.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_PipelineCache(VkPipelineCache var) {
    TrimObjectInfoMap::iterator iter = createdPipelineCaches.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_DescriptorPool(VkDescriptorPool var) {
    TrimObjectInfoMap::iterator iter = createdDescriptorPools.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_Pipeline(VkPipeline var) {
    TrimObjectInfoMap::iterator iter = createdPipelines.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_Semaphore(VkSemaphore var) {
    TrimObjectInfoMap::iterator iter = createdSemaphores.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_Fence(VkFence var) {
    TrimObjectInfoMap::iterator iter = createdFences.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_Framebuffer(VkFramebuffer var) {
    TrimObjectInfoMap::iterator iter = createdFramebuffers.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_Event(VkEvent var) {
    TrimObjectInfoMap::iterator iter = createdEvents.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_QueryPool(VkQueryPool var) {
    TrimObjectInfoMap::iterator iter = createdQueryPools.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
ObjectInfo *StateTracker::get_DescriptorSet(VkDescriptorSet var) {
    TrimObjectInfoMap::iterator iter = createdDescriptorSets.find(reinterpret_cast<uint64_t>(var));
    ObjectInfo *pResult = NULL;
...
void StateTracker::remove_Instance(VkInstance var) {
    ...
    createdInstances.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_PhysicalDevice(VkPhysicalDevice var) {
    ...
    createdPhysicalDevices.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_Device(VkDevice var) {
    ...
    createdDevices.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_SurfaceKHR(VkSurfaceKHR var) {
    ...
    createdSurfaceKHRs.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_Queue(VkQueue var) {
    ...
    createdQueues.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_CommandPool(VkCommandPool var) {
    ...
    createdCommandPools.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_SwapchainKHR(VkSwapchainKHR var) {
    ...
    createdSwapchainKHRs.erase(reinterpret_cast<uint64_t>(var));
}

void StateTracker::remove_CommandBuffer(VkCommandBuffer var) {
    createdCommandBuffers.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_DeviceMemory(VkDeviceMemory var) {
    ...
    createdDeviceMemorys.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_Image(VkImage var) {
    ...
    createdImages.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_ImageView(VkImageView var) {
    ...
    createdImageViews.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_Buffer(VkBuffer var) {
    ...
    createdBuffers.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_BufferView(VkBufferView var) {
    ...
    createdBufferViews.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_Sampler(VkSampler var) {
    ...
    createdSamplers.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_DescriptorSetLayout(VkDescriptorSetLayout var) {
    ...
    createdDescriptorSetLayouts.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_PipelineLayout(VkPipelineLayout var) {
    ...
    createdPipelineLayouts.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_RenderPass(VkRenderPass var) {
    ...
    createdRenderPasss.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_ShaderModule(VkShaderModule var) {
    ...
    createdShaderModules.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_PipelineCache(VkPipelineCache var) {
    ...
    createdPipelineCaches.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_Pipeline(VkPipeline var) {
    ...
    createdPipelines.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_DescriptorPool(VkDescriptorPool var) {
    ...
    createdDescriptorPools.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_DescriptorSet(VkDescriptorSet var) {
    ...
    createdDescriptorSets.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_Framebuffer(VkFramebuffer var) {
    ...
    createdFramebuffers.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_Semaphore(VkSemaphore var) {
    ...
    createdSemaphores.erase(reinterpret_cast<uint64_t>(var));
}

void StateTracker::remove_Fence(VkFence var) { createdFences.erase(reinterpret_cast<uint64_t>(var)); }
...
void StateTracker::remove_Event(VkEvent var) {
    ...
    createdEvents.erase(reinterpret_cast<uint64_t>(var));
}
...
void StateTracker::remove_QueryPool(VkQueryPool var) {
    ...
    createdQueryPools.erase(reinterpret_cast<uint64_t>(var));
}
}
