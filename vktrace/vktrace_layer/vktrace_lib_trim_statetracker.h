...
class StateTracker {
  public:
    ...
    std::unordered_map<VkCommandBuffer, std::list<ImageTransition>>
        m_cmdBufferToImageTransitionsMap;
    void AddImageTransition(VkCommandBuffer commandBuffer,
                            ImageTransition transition);
    void ClearImageTransitions(VkCommandBuffer commandBuffer);

    std::unordered_map<VkCommandBuffer, std::list<BufferTransition>>
        m_cmdBufferToBufferTransitionsMap;
    ...
    VkRenderPassCreateInfo *get_RenderPassCreateInfo(VkRenderPass renderPass,
                                                     uint32_t version);
    ...
    std::unordered_map<VkCommandBuffer,
                       std::list<vktrace_trace_packet_header *>>
        m_cmdBufferPackets;

    // Map to keep track of older RenderPass versions so that we can recreate
    // pipelines.
    std::unordered_map<VkRenderPass, std::vector<VkRenderPassCreateInfo *>>
        m_renderPassVersions;
