...
{
    VkApplicationInfo *p = (VkApplicationInfo *)struct_ptr;  // Change this to generic struct

    // What if we translate handles in this struct too??
    if (p)
        p = (VkApplicationInfo *)p->pNext;
    ...
    while (p)
    {
        ...
        case VK_STRUCTURE_TYPE_DEVICE_GROUP_DEVICE_CREATE_INFO:
            {
            // Need to loop on physicalDeviceCount
            VkDeviceGroupDeviceCreateInfo *p2 = (VkDeviceGroupDeviceCreateInfo *)p;
            *((VkPhysicalDevice *)p2->pPhysicalDevices) = m_objMapper.remap_physicaldevices(*p2->pPhysicalDevices);
            }
            printf("FOUND A DGDCI\n"); fflush(stdout);
            break;
        default:
            break;
        }
